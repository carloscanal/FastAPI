...
path = "/api/v2/"                                   # evita colision de paths (/hola)
...
async def hola(nombre : str, saludo : Union[str, None] = None):
    ...
    if saludo != None :                            # endpoint: api/v2/hola/{nombre}?saludo=<saludo>
        saludo = ", " + saludo                     #     p.ej. api/v2/hola/Pepe?saludo=qué%20tal%20estás?
    else:
        saludo = ""
...
async def create_item(item: Item):
    ...
    return {"message": item.nombre + " nos dice lo siguiente: " + item.descripcion + ", por " + str(item.precio) + " euros."}
